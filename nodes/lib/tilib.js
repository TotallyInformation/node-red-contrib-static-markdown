...
module.exports = {
    ...
    /** Joins all arguments as a filing system path string - ES6+ only
     * @arguments {array|string} arrays or strings to merge
     * @returns {string}
     */
    pathJoin: function(...args) {
        // Reduce all supplied args to a single array
        const paths = args.reduce((acc, val) => {
            // If an arg is not an array, make it one
